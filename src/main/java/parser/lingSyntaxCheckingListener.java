...
public class lingSyntaxCheckingListener extends lingBorBaseListener{
    ...
    private void printMap(HashMap<String, Symbol> mp){
        ...
        System.out.println(String.format("<--------------Symbol__Table size:%d---------------->",mp.size()));
    ...
    public void printSymbolMap(){
        if (statusFunc == 0){
            ...
            System.out.println("\n<----------------size: %d----------------->");
            System.out.println("///>NOW WE ARE IN THE SCOPE OF A FUNCTION<///");
            printMap(funcVarMap);
        }
    }
